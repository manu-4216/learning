...
type Record<Indexes extends string | number | symbol, MappedType> = {
  [P in Indexes]: MappedType;
};
...
type PickedValues = Pick<{ name: string; color: string; sweetness: number }, 'name' | 'color'>;
// type PickedValues = {
//   name: string,
//   color: string
// }
...
type OmittedValues = Omit<{ name: string; color: string; sweetness: number }, 'name' | 'color'>;
// type OmittedValues = {
//   sweetness: number,
// }
...
type Parameters<T extends Function> = T extends (...args: infer Args) => any ? Args : never;
...
// constraint that ensures it is a function.
type ReturnType<T extends Function> = T extends (...args: any) => infer Return ? Return : never;
type FunctionReturn = ReturnType<typeof parseFloat>;
// type FunctionReturn = number

// Interesting use-case of using the built-in Awaited, ReturnType:
type Post = {
  id: number;
  title: string;
};
